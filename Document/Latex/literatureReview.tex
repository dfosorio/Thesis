...
Generally speaking, the blueprint to modify and run any PMaude model to be used by MultiVeStA is:
\begin{enumerate}
    \item Define the PMaude model states as configurations using object-oriented programming in Maude and define the system transitions as rewriting rules between configurations.
    \item Include the necessary elements of Multivesta inside the model's
    Maude definition.
...
\end{enumerate}
We will walk-through this steps with a practical example, where we aim to verify properties over the PMaude model of the clock model presented in section 2.6. Before we start, it is important to mention that the version of the MultiVeStA tool used for this example can be found in \cite{multiTool}. In this page the reader can find the instructions to download and run a prepared experiment with the tool.

\subsection{Step 1}
First of all, we will start from the following specification of the clock model explained in section 2.6 (named as \texttt{clock.maude}) taken from \cite{MaudeManual}:
\begin{lstlisting}
mod CLOCK is 
  pr SAMPLER . 
  sort Clock . 
...
endm
\end{lstlisting}
To transform this module into an object oriented module, we will divide the sepecification in different modules. The first one is the module \texttt{SORTS}, where the different sorts of the attributes of the configuration or class will be specified. In this case, a sort \texttt{State} that has two constants \texttt{working} and \texttt{broken}, that represent the clock's state:
\begin{lstlisting}
mod SORTS is
  pr APMAUDE .
...
endm
\end{lstlisting}
The second module is \texttt{ATTRIBUTES}, where the attributes of the model's states are defined:
\begin{lstlisting}
mod ATTRIBUTES is
  pr NAT .
  pr SORTS .
...
endm
\end{lstlisting}
The third module is \texttt{SCENARIO}
